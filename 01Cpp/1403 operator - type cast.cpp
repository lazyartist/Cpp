...
void _1403_operator_type_cast() {
	...
	public:
		...
		// int로 A 객체 생성
		explicit A(int i) { // 명시적 생성자 호출만 가능
	...
	cout << "===== Type Cast by constructor  =====" << endl;
	{
		...
		a = b; // 암시적 생성자 호출
		// B 클래스를 인자로 받는 생성자가 있으므로 컴파일러는 A(B b)를 호출해 A 클래스의 임시객체를 생성한다.
		// 클래스 A에 operator= 재정의 되지 않았으므로 컴파일러에 정의된 =연산자로 변수 a에 값이 복사된다.(이때 A(const A& a) 생성자가 호출되지 않는다.)
		// 이 라인이 끝났으므로 임시 객체가 소멸된다.
		cout << "-----" << endl;
		/*
		A 010FFCD0, 0
		B 010FFCC4
